...
if(isset($_POST['aluno'])){
    ...
    $sql_alu   = "select matricula_alu, nome_alu, sobrenome_alu, usuario_alu, senha_alu, email_alu, nivel_alu from aluno";
    ...
    if(mysqli_num_rows($query_alu) != 1){
    ...
    }elseif(sha1($senha) == sha1($dados_alu['matricula_alu'])){
        session_start();
        ...
        header("Location: ../model/aluno/login_alu/primeiro_acesso.php");
    }else{
        session_start();
        $_SESSION['matricula_alu'] = $dados_alu['matricula_alu'];
        $_SESSION['nome_alu'] = $dados_alu['nome_alu'];
